...
\text{ind=}
\if{ \op=4 && \rep5=4}{
   \text{ind=On peut lever l'indétermination en écrivant :<br />
...
\statement{
<p> 
Soit \(f) la fonction   définie sur \(\ensdef) par :
\(f(x) = \tf).
</p><p>
On a donc \(f = \forme )  avec, pour tout réel  \(x) de \(\ensdef), 
\(u(x) = \u)  &nbsp; et &nbsp; \(v(x) = \v).
</p><p> 
...
\if{\step=2}
    {\if{\sc_reply1=1}
     {<span class="oef_indgood">Votre réponse était juste!</span><br />}
     {<span class="oef_indbad">Attention, votre réponse (\reply1) n'était pas la bonne!</span><br />}
}
...
\if{\step=3}
    {\if{\sc_reply2=1}
     {<span class="oef_indgood">Votre réponse était juste!</span><br />}
     {<span class="oef_indbad">Hélas, votre réponse (\reply2) n'était pas bonne !</span><br />}
}
...
{<li>
    \if{\sc_reply3=1}
     {<span class="oef_indgood">Votre réponse était juste!</span><br />}
     {<span class="oef_indbad">Humm, votre réponse (\reply3) n'était pas correcte !</span><br />}

En posant \(t = \u), sachant que \(\displaystyle{\lim_{t\to\sol2} \g  = \sol3}), on déduit que : 
...
\if{\step=5}
    {\if{\sc_reply4=1}
     {<span class="oef_indgood">Votre réponse était juste!</span><br />}
     {<span class="oef_indbad">Votre réponse (\reply4) n'était pas correcte !</span><br />}
}
...
\if{\step=5}
{<li> Peut-on  déduire la limite en \(\texlim)  de \(f = \forme) en appliquant les règles opératoires sur les limites ? 
...
{<li>
   ...
   \if{\sc_reply5=1}
     {<span class="oef_indgood">Votre réponse était juste!</span><br />}
     {<span class="oef_indbad">Votre réponse (\reply5)  était erronée !</span><br />}
...
\answer{forme indéterminée :}{\rep5;\typeindet}{type=select}{option=nonstop}
...
écrire <b>+inf</b> pour désigner \(+\infty)  ;<br/> écrire  <b>-inf</b> pour désigner \(-\infty). 
   }
}
