...
%% cas 2 pour déterminer si borne infinie ou pas
%% cas2=1 +inf, =2 -inf =3 ou 4 bornée
...
\text{choixrepM= oui, non } %% majorée
\text{choixrepm= oui, non } %% minorée
\text{borne2=minimum}
...
%% cadre générique %%
%%%%%%%%%  adaptation aux valeurs de l'énoncé
...
\text{textesol= La fonction possède un maximum qui vaut \valb puisque :
<ul><li>toute la courbe se situe en dessous de la droite d'équation \(y=\valb) (en pointillés bleus sur le  graphique)
</li><li>le point de coordonnnées \((\valbx ; \valb)) appartient à la courbe.
</li></ul> }}
{
{\text{seuilmax=dhline 1,\valb2,blue}
\text{textesol2= La fonction possède un maximum qui vaut \valb2 puisque :
<ul><li>toute la courbe se situe en dessous de la droite d'équation \(y=\valb2) (en pointillés bleus sur le  graphique)
</li><li>le point de coordonnnées \((\valb2x ; \valb2)) appartient à la courbe.
</li></ul> }
}
}
}
{\text{textesol3=La fonction ne possède pas de maximum. En effet, sur le graphique, aucune droite parallèle à l'axe des abscisses n'est au dessus de toute la courbe.}}
%% affichage pour le min %%%%%%%%%%
\if{\indrepm=1}{
\if{\borne==minimum}{
\text{seuilmin= dhline 1,\valb,green}
\text{textesol= La fonction possède un minimum qui vaut \valb puisque :
<ul><li>toute la courbe se situe au dessus de la droite d'équation \(y=\valb) (en pointillés verts sur le graphique)
</li><li>le point de coordonnnées \((\valbx ; \valb)) appartient à la courbe.
</li></ul> }
}
{\text{seuilmin=dhline 1,\valb2,green}
\text{textesol2= La fonction possède un minimum qui vaut \valb2 puisque :
<ul><li>toute la courbe se situe au dessus de la droite d'équation \(y=\valb2) (en pointillés verts sur le graphique)
</li><li>le point de coordonnnées \((\valbx2 ; \valb2)) appartient à la courbe.
</li></ul> }
}
}
{\text{textesol4=La fonction ne possède pas de minimum. En effet, sur le graphique, aucune droite parallèle à l'axe des abscisses n'est au dessous de toute la courbe.}}
...
<p>
On cherche à étudier ses extrema éventuels par lecture graphique.</p>
\trace

\if{\step=1}{<div class=wims_question>Complétez :<br />
