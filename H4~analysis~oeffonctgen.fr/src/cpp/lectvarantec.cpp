...
Trouver un antécédent de \xl[\indl[1]] par la fonction \(f) : \embed{reply1}
}

\answer{antécédent de \xl[\indl[1]]}{\xl[\indl[2]]}{type=numeric}
\hint{ On lit les images dans la partie \(f(x)) du tableau.
<br />
On lit les antécédents sur la ligne des \(x) du tableau.}

\feedback{\reply1=\badrep and \reply<>\xl[\indl[2]]}{ Attention, vous avez lu l'image de \xl[\indl[1]]!
Il faut chercher la valeur \xl[\indl[1]] dans la partie \(f(x))
du tableau et lire son antécédent dans la ligne des \(x) du tableau.}
