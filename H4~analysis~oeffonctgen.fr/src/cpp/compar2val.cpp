...
\text{slt=\(<)}
\text{sgt=\(>)}
\text{choixcomp=inferieur à,supérieur à,on ne peut pas savoir}
...
\statement{
<p>
Soit \(f) une fonction définie sur \crog \x1;\x4 \crod dont le
...
On cherche à comparer certaines images par \(f).
